...
<section class="min-h-screen py-16 px-4 sm:px-6 lg:px-0" x-data="{ selectedOS: 'linux', isVisible: false }" x-init="setTimeout(() => isVisible = true, 100)">
    <div class="max-w-7xl mx-auto">
        {{-- Header --}}
        <div class="text-center mb-16 transition-all duration-1000" :class="{'opacity-100 translate-y-0': isVisible, 'opacity-0 translate-y-8': !isVisible}">
            ...
            <div class="flex justify-center mb-8">
                <div class="bg-white rounded-full shadow-lg p-1 border border-blue-200">
                    <div class="flex">
                        <button @click="selectedOS = 'linux'"
                            class="px-6 py-2 text-sm rounded-full font-semibold transition-all duration-300"
                            :class="{'bg-blue-600 text-white shadow': selectedOS === 'linux', 'text-blue-700 hover:bg-blue-50': selectedOS !== 'linux'}">
                            Linux
                        </button>
                        <button @click="selectedOS = 'windows'"
                            class="px-6 py-2 text-sm rounded-full font-semibold transition-all duration-300"
                            :class="{'bg-blue-600 text-white shadow': selectedOS === 'windows', 'text-blue-700 hover:bg-blue-50': selectedOS !== 'windows'}">
                            Windows
                        </button>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <template x-for="plan in (selectedOS === 'linux' ? {{ json_encode($linuxPlans) }} : {{ json_encode($windowsPlans) }})" :key="`${selectedOS}-${plan.name}`">
                <div class="relative bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 hover:scale-105"
                    :class="{'ring-4 ring-green-500 ring-opacity-80': plan.popular, 'opacity-100 translate-y-0': isVisible, 'opacity-0 translate-y-8': !isVisible}"
                    :style="`transition-delay: ${($store.alpine.asArray($data.plans).indexOf(plan) * 150)}ms`">
                    {{-- Popular Badge --}}
                    ...
                    <div class="p-8">
                        ...
                        {{-- CTA Button --}}
                        <a :href="`{{ url('/billing') }}?plan=${encodeURIComponent(JSON.stringify({...plan, category: 'VPS-Foreign Hosting', type: 'monthly'}))}`"
        ...
        {{-- Bottom Section --}}
        <div class="text-center mt-20 transition-all duration-1000 delay-1000" :class="{'opacity-100 translate-y-0': isVisible, 'opacity-0 translate-y-8': !isVisible}">
